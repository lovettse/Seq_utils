...
Future features:
  ...
  Stop indexing the reference twice
  Print depth plot commands during dry run

Necessary inputs:
  Reference fasta (edit headers to only include accession number)
  GFF file (accession must match ref fasta)
  Reads (must be paired-end, may be gzipped)
...
Changelog
Current Version
  - Added handling of single end reads. Will only work with only single-end reads. Could use improvement.

v1.0
  - Deprecated "--use_db" option to eliminate potential for mismatch between provided GFF and snpEff db.
  - Added "--organize" option.
  - Now handles gzipped input fastqs
  - Improved handling of dry_run command printing
'''
...
def main():
  ...
  input_opts.add_option('--single', default=False, action="store_true", help= "Input is single-end (changes default value for --cut) [False]")
  ...
  func_opts.add_option('--organize', default=False, action="store_true", help="EXPERIMENTAL Organize directory after run. [False]")
  ...
  param_opts.add_option('-m', '--minlen', default=70, type="int", help="Minimum read length after filtering [70]")
  param_opts.add_option('--mapq', type="int", default=10, help="Minimum mapping quality to keep read [10]")
  # param_opts.add_option('--use_db', help="snpEff db to use. One will be created from gff if not set [None]") # Deprecated. opt.gff should be used alone.
  ...
  param_opts.add_option('--improper', default=False, action="store_true", help="Allow imporperly paired reads in bowtie alignment. [False]")
  ...
  out_opts.add_option('-b', '--bam', default="merged.bam", help="Name for output merged bam. Basename will also be used for VCF and summary files [merged.bam]")
  ...
  if opts.single and opts.improper: print("Cannot use improperly paired reads with single-end data")
  if opts.input is None or opts.ref is None or opts.gff is None or (opts.single and opts.improper):
    print("Use '-h' for help")
    exit(-1)

  # If running in single end mode, the default cutadapt cannot be used
  if opts.single and opts.cut == "/data/_scripts/slovett/cutadapt_paired_nosispa_2.0.py":
    opts.cut = "/home/slovett/scripts/cutadapt_illumina_single.py"
  ...
  for sample in sorted(reads):
    ...
    if opts.single:
      read1_cut = run_cutadapt(sample, reads[sample], 0, opts)
      read1_ps = run_prinseq(sample, read1_cut, 0, opts)
      bam = run_align(sample, ref_base, read1_ps, 0, opts)
    else:
    ...
    if opts.single:
      files_to_remove.extend((read1_cut, bam, rmdups))
    else:
  ...
  if opts.depth:
    ...
    if opts.dry_run:
      print("# These commands aren't printed during a dry run")
  ...
  snpeff_db = make_snpeff_db(opts)
  snpeff_vcf = run_snpeff(snpeff_db, vcf_file, opts)
  print("")
  ...
  if not opts.dry_run:
    final_vcf = add_lc_rep_annots(summary_vcf, lc_annot, rep_annot)
  print("")
...
def organize(opts):
  mkdir_cline="mkdir alignments plots depths variant_calls filtered_reads metrics"
  mv_clines=("mv *.bam alignments", "mv *covplots.pdf plots", "mv *.depth depths", "mv *vcf* variant_calls", "mv *_cutadapt_prinseq_?.fastq filtered_reads", "mv *bowtie.out *prinseq.err *.metrics metrics")
  print(mkdir_cline)
  
  if not opts.dry_run:
    Popen(mkdir_cline, shell=True, stdout=PIPE, stderr=PIPE)
  
  for cline in mv_clines:
    print(cline)
    if not opts.dry_run:
...
def cleanup(files_to_remove, opts):
  cline = "rm %s *.bai *.dict *.fai *singletons.fastq *bt2 *.sam snpEff_genes.txt snpEff_summary.html *.depths" % " ".join(files_to_remove)
  print(cline) 
...
def add_lc_rep_annots(vcf, lc_annot, rep_annot):
  ...
  with open(vcf,"r") as vcf_in:
    with open(out_name,"w") as vcf_out:
      i=0
      with open(lc_annot, "r") as lc_in:
        lc_annots = lc_in.readlines()
      with open(rep_annot, "r") as rep_in:
        rep_annots = rep_in.readlines()
      for line in vcf_in:
        if line.startswith("#"):
          vcf_out.write(line)
...
def make_snpeff_db(opts):
  db_name = ".".join(opts.ref.split(".")[:-1]).split("/")[-1]
...
def call_vars(bam, opts):
  ...
  clines.append("%s -Xmx48g -jar %s -T RealignerTargetCreator -nt %s -R %s -I %s -o realignment_targets.list" % (opts.java, opts.gatk, opts.threads, opts.ref, bam))

  realigned_name = "%s_realigned.bam" % ".".join(bam.split(".")[:-1])
  clines.append("%s -Xmx48g -jar %s -T IndelRealigner -R %s -I %s -targetIntervals realignment_targets.list -o %s" % (opts.java, opts.gatk, opts.ref, bam, realigned_name))
  
  out_name = "%s_variants.vcf" % ".".join(bam.split(".")[:-1])
  clines.append("%s -Xmx32g -jar %s -T UnifiedGenotyper -R %s -I %s -o %s -ploidy 1 -glm both -nt %s" % (opts.java, opts.gatk, opts.ref, realigned_name, out_name, opts.threads))
...
def run_align(sample, base, read1, read2, opts):
  ...
  if not opts.single:
    if opts.improper:
      bowtie_cline = "%s -x %s -1 %s -2 %s -p %s -X 1000 2> %s_bowtie.out | %s view -q %s -F 4 -bS - | %s sort -@ %s -o %s -" % (opts.bowtie, base, read1, read2, opts.threads, sample, opts.samtools, opts.mapq, opts.samtools, opts.threads, out_name)
    else:
      ...
      bowtie_cline = "%s -x %s -U %s -p %s 2> %s_bowtie.out | %s view -q %s -F 4 -bS - | %s sort -@ %s -o %s -" % (opts.bowtie, base, read1, opts.threads, sample, opts.samtools, opts.mapq, opts.samtools, opts.threads, out_name)
  print(bowtie_cline)
  ...
  if not read1.endswith("gz"):
    if opts.single:
      pigz_cline = "pigz %s" % (read1)
    else:
      pigz_cline = "pigz %s %s" % (read1, read2)
    print(pigz_cline)
    pigz_run=Popen(pigz_cline, shell=True, stdout=PIPE,stderr=PIPE)
...
def run_prinseq(sample, read1, read2, opts):
  if opts.single:
    ...
    cline = "%s -fastq %s -out_bad null -out_good %s -min_len %s -trim_qual_right 30 -trim_qual_type min -trim_qual_window 5 -min_qual_mean 20 2> %s_prinseq.err" % (opts.prinseq, read1, out_name, opts.minlen, sample)
    print(cline)
  
    read1_ps = "%s.fastq" % out_name
...
def run_cutadapt(samplename, read1, read2, opts):
  if not opts.single:
    ...
    # The command line has already been written, and the cutadapt script handles gzipped files
    # If I take off ".gz" now, gzipped fastqs will work
    if read1.endswith("gz"):
      read1 = ".".join(read1.split('.')[:-1])
    if read2.endswith("gz"):
      read2 = ".".join(read2.split('.')[:-1])
    ...
    cline = "%s -1 %s -l %s" % (opts.cut, read1, opts.minlen)
    print(cline)
    ...
    read1_cut = '%s_cutadapt.fastq' % '.'.join(read1.split('.')[:-1])
