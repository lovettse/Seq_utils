...
def main():
  ...
  p.add_option('-s', '--sort', help="Path to order_fasta_by_length.sh [/data/_scripts/slovett/order_fasta_by_length.sh]")
  
  opts, args = p.parse_args()
  
  run_len_sort(opts)

  fasta_dict = read_fasta_dict("temp.fasta")
  
  write_fasta_from_dict(fasta_dict, opts.output)
  
def run_len_sort(opts):
  cline="%s %s > temp.fasta" % (opts.sort, opts.input)
  print(cline)
  
  run_lensort = Popen(cline, shell=True, stdout=PIPE, stderr=PIPE)
  run_lensort.wait()
